...
export class VoiceBotService {
  ...
  private async setupTimerNotifyCallback() {
    ...
    setTimerNotifyCallback(
      async (roomId: string, event: "warning" | "ended", data: any) => {
        ...
        this.logger.log(
          `[타이머 콜백] ${roomId} - ${event}, 남은 시간: ${data.remainingSeconds}초`,
        );
        ...
        try {
          ...
          this.logger.error(
            `[타이머 콜백] DataChannel 전송 실패: ${error.message}`,
          );
        }
      },
    );
  ...
  async startBot(roomId: string, botToken: string): Promise<void> {
    ...
    room.on(
      ...
      async (
        ...
        const isScreenShareAudio =
          publication.source === TrackSource.SOURCE_SCREENSHARE_AUDIO;
        if (
          ...
          this.logger.log(
            `[오디오 트랙 구독] ${participant.identity} (source: ${publication.source})`,
          );
    ...
    room.on(
      ...
      async (payload: Uint8Array, participant?: RemoteParticipant) => {
        try {
          ...
          this.logger.debug(
            `[DataChannel] 메시지 수신 from ${participant?.identity || "unknown"}: type=${message.type}`,
          );
          if (message.type === "vision_capture_response") {
            this.logger.log(
              `[Vision] 캡처 응답 수신 - requestId: ${message.requestId}, 이미지 크기: ${(message.imageBase64?.length / 1024).toFixed(1)}KB`,
            );
          ...
          if (message.type === "TIMER_COMMAND") {
            this.logger.log(
              `[타이머 명령 수신] action: ${message.action}, minutes: ${message.minutes}`,
            );
    ...
    room.on(
      ...
      async (participant: RemoteParticipant) => {
        ...
        if (!participant.identity.startsWith("ai-bot")) {
          ...
          if (
            context?.hostOnlyMode &&
            !context.hostIdentities.has(participant.identity)
          ) {
            this.ragClient
              .participantJoined(roomId, {
          ...
          try {
            ...
            if (timerStatus && timerStatus.timerState !== "idle") {
              ...
              this.logger.log(
                `[타이머 동기화] 새 참여자 ${participant.identity}에게 타이머 상태 전송`,
              );
            }
          } catch (error) {
      ...
      (participant: RemoteParticipant) => {
        ...
        if (!participant.identity.startsWith("ai-bot")) {
          ...
          if (context?.hostOnlyMode) {
            this.ragClient
              .participantLeft(roomId, participant.identity)
              .catch((err) => {
        ...
        const humanCount = Array.from(room.remoteParticipants.values()).filter(
          (p) => !p.identity.startsWith("ai-bot"),
        ).length;
    ...
    room.on(RoomEvent.Disconnected, (reason?: any) => {
      this.logger.error(
        `[봇 연결 끊김] roomId=${roomId}, 사유: ${reason || "UNKNOWN"}`,
      );
    ...
    room.on(RoomEvent.DataReceived, (payload: Uint8Array, participant: any) => {
      try {
        ...
        this.logger.debug(
          `[DataChannel 수신] type=${message.type}, from=${participant?.identity || "unknown"}`,
        );

        const context = this.activeRooms.get(roomId);
        if (!context) {
          this.logger.warn(
            `[DataChannel] context 없음 - 메시지 무시: ${message.type}`,
          );
          return;
        }
        ...
        if (message.type === "CLIENT_READY") {
          if (!context.greetingDone) {
            this.logger.log(
              `[CLIENT_READY] 클라이언트 준비 완료 - 인삿말 시작`,
            );
        ...
        if (message.type === "AI_MUTE") {
          ...
          this.logger.log(
            `[AI 음소거] ${muted ? "ON (오디오 중단)" : "OFF"} by ${participant?.identity || "unknown"}`,
          );
          return;
        }
        ...
        if (message.type === "HOST_MODE_ENABLE") {
          ...
          this.logger.log(
            `[호스트 모드] ${context.hostOnlyMode ? "ON" : "OFF"} - hosts: ${Array.from(context.hostIdentities).join(", ")}`,
          );
          ...
          if (context.hostOnlyMode && hostId) {
            ...
            // 기존 참여자들도 전송 (호스트 제외)
            for (const [identity, remoteParticipant] of context.room
              .remoteParticipants) {
              if (
                !context.hostIdentities.has(identity) &&
                !identity.startsWith("ai-bot")
              ) {
                this.ragClient
                  .participantJoined(roomId, {
                    id: identity,
        ...
        if (message.type === "HOST_ADD" && message.host) {
          ...
          context.hostIdentities.add(hostId);
          this.logger.log(
            `[호스트 추가] ${hostId} - 현재 호스트: ${Array.from(context.hostIdentities).join(", ")}`,
          );
          ...
          this.ragClient
            ...
            .catch((err) => {
              this.logger.warn(
                `[RAG 호스트 역할 변경] 전송 실패: ${err.message}`,
              );
            });
          return;
        }
        ...
        if (message.type === "HOST_REMOVE" && message.identity) {
          ...
          this.logger.log(
            `[호스트 제거] ${hostId} - 현재 호스트: ${Array.from(context.hostIdentities).join(", ")}`,
          );

          // RAG 서버에 역할 변경 전송
          const removedParticipant =
            context.room.remoteParticipants.get(hostId);
          this.ragClient
            .participantJoined(roomId, {
              id: hostId,
            ...
            .catch((err) => {
              this.logger.warn(
                `[RAG 참여자 역할 변경] 전송 실패: ${err.message}`,
              );
            });
          return;
        }
        ...
        if (message.type === "WAKE_WORD_TOGGLE") {
          ...
          if (
            ...
            this.logger.log(
              `[Wake Word] ${context.wakeWordEnabled ? "ON" : "OFF"} by host`,
            );
          }
          return;
        }
        ...
        if (message.type === "IDEA_MODE_START") {
          ...
          this.logger.log(
            `[모드 변경] 아이디어 모드 ON by ${participant?.identity || "unknown"}`,
          );
          ...
          if (
            ...
            now - context.flowchartOpenTime < OPEN_GRACE_PERIOD_MS
          ) {
          ...
          // CDR → Flowchart 변환 요청
          this.logger.log(
            `[CDR→Flowchart] 요청 수신 - ${message.content?.length || 0}자`,
          );
          ...
          this.logger.log(
            `[CDR 분석] 요청 수신 - ${message.content?.length || 0}자`,
          );
          ...
          // Skeleton 코드 생성 요청
          this.logger.log(
            `[Skeleton] 요청 수신 - ${message.nodes?.length || 0}개 노드`,
          );
          ...
          this.logger.log(
            `[Flowchart 컨텍스트] 업데이트 - 노드: ${message.nodes?.length || 0}, 엣지: ${message.edges?.length || 0}`,
          );
          ...
          if (!context.flowchartModeActive && message.nodes?.length > 0) {
            context.flowchartModeActive = true;
            this.logger.log(
              `[모드 변경] Flowchart 자동 활성화 (컨텍스트 수신)`,
            );
          }
          ...
          this.logger.log(
            `[모드 변경] 설계 모드 ON by ${participant?.identity || "unknown"}`,
          );
          ...
          if (
            ...
            now - context.designModeOpenTime < DESIGN_GRACE_PERIOD_MS
          ) {
          ...
          this.logger.log(
            `[설계 모드] 다이어그램 생성 완료 - types: ${message.types?.join(", ")}`,
          );
          if (context.designModeActive && message.types?.length > 0) {
            ...
            // 자동 설명 실행
            this.explainGeneratedDiagrams(
              roomId,
              message.types,
              message.mermaidCodes || {},
            );
          }
        } else if (message.type === "PDF_QUESTION") {
          // PDF 텍스트 선택 후 AI 질문
          this.logger.log(
            `[PDF 질문] 수신 - question: "${message.question?.substring(0, 50)}...", context: ${message.context}`,
          );
          ...
          this.logger.log(
            `[공유 보드 변환] 요청 수신 - 이미지 크기: ${(message.imageBase64?.length / 1024).toFixed(1)}KB`,
          );
          ...
          // 스터디 퀴즈 생성 요청
          this.logger.log(
            `[스터디 퀴즈] 생성 요청 - 문제수: ${message.questionCount}, 유형: ${message.quizType}, QA히스토리: ${message.qaHistory?.length || 0}개`,
          );
          ...
          // 퀴즈 답변 평가 요청
          this.logger.log(
            `[스터디 퀴즈] 평가 요청 - 답변: "${message.userAnswer?.substring(0, 30)}..."`,
          );
        ...
        // ============================================================
          ...
          room.remoteParticipants.forEach((p) => {
            if (!p.identity.includes("bot") && !p.identity.includes("agent")) {
              this.perplexityService.addParticipant(
                roomId,
                p.identity,
                p.name || p.identity,
              );
          ...
          if (
            participant &&
            !addedParticipants.includes(participant.identity)
          ) {
            if (
          ...
          this.logger.log(
            `[Perplexity] 모드 ON by ${participant?.identity || "unknown"} - 참여자: ${addedParticipants.join(", ")}`,
          );
    ...
    try {
      ...
      this.ragClient
        ...
        .catch((error) =>
          this.logger.warn(
            `[RAG 연결 실패] ${roomId}: ${error.message} - RAG 없이 계속 진행`,
          ),
        );

      const audioSource = new AudioSource(16000, 1);
      const localAudioTrack = LocalAudioTrack.createAudioTrack(
        "ai-voice",
        audioSource,
      );
      ...
      this.logger.log(
        `[봇 입장 성공] 참여자: ${room.remoteParticipants.size}명`,
      );

      // 입장 인사는 클라이언트가 CLIENT_READY 메시지를 보낼 때까지 대기
      this.logger.log(
        `[봇 입장] 클라이언트 준비 대기 중 (CLIENT_READY 메시지 대기)`,
      );
  ...
  private async detectAndBroadcastIdea(
    ...
    // 단순 인사/추임새 필터링 (최소한만)
    const skipPatterns =
      /^(안녕|네|응|어|음|아|예|오케이|ㅇㅇ|ㅋ+|하하|그래|알겠|뭐지|뭐야|아아|으으|에에)[\.\?\!]?$/i;
  ...
  private extractIdeaContent(transcript: string): string {
    ...
    text = text.replace(/[\?\!\.]+$/, "");
    text = text.replace(
      /(은|는)?\s*(어때|어떨까|어떨까요|어때요|좋을까|좋겠다|하면|해볼까|해보자|하자)\s*$/g,
      "",
    );
    ...
    // 불필요한 접두사 제거
    text = text.replace(
      /^(그|저|이|그러면|아|어|음|근데|그래서|그런데)\s+/g,
      "",
    );
  ...
  private readonly FLOWCHART_NODE_TYPES = [
    "start",
    "end",
    "process",
    "decision",
...
{"nodes":[{"id":"n1","content":"시작","nodeType":"start"},{"id":"n2","content":"입력","nodeType":"io"},{"id":"n3","content":"조건?","nodeType":"decision"},{"id":"n4","content":"처리A","nodeType":"process"},{"id":"n5","content":"처리B","nodeType":"process"},{"id":"n6","content":"종료","nodeType":"end"}],"edges":[{"from":"n1","to":"n2"},{"from":"n2","to":"n3"},{"from":"n3","to":"n4","label":"Yes"},{"from":"n3","to":"n5","label":"No"},{"from":"n4","to":"n6"},{"from":"n5","to":"n6"}],"summary":"조건 분기 흐름"}`;
      ...
      this.logger.log(
        `[CDR→Flowchart] LLM 호출 시작 (${processedContent.length}자)`,
      );
      const answer = await this.llmService.sendMessagePure(prompt, 2000);
      this.logger.debug(
        `[CDR→Flowchart] LLM 응답: ${answer.substring(0, 200)}...`,
      );
      ...
      const rawNodes = (parsed.nodes || []).filter(
        (n: any) =>
          ...
          n.id &&
          n.content &&
          this.FLOWCHART_NODE_TYPES.includes(n.nodeType),
      );
      ...
      this.logger.log(
        `[CDR→Flowchart 완료] ${nodes.length}개 노드, ${edges.length}개 연결`,
      );
  ...
  private async handlePdfQuestion(
    ...
    this.logger.log(
      `[PDF 질문] 처리 시작 from ${requesterId}: "${question.substring(0, 50)}..."`,
    );
...
`.trim();
      ...
      this.logger.log(
        `[PDF 질문] 응답 전송 완료 (to: ${requesterId}): ${response.substring(0, 50)}...`,
      );
      ...
      try {
        ...
        // 연결 끊김 등의 에러는 무시
        this.logger.debug(
          `[PDF 질문] 에러 응답 전송 실패 (연결 끊김): ${publishError.message}`,
        );
      }
    }
  }
...
손글씨가 알아보기 어려우면 최대한 추측해서 변환하세요. 그림이 명확하지 않으면 기본 흐름도로 만들어주세요.`;
      ...
      this.logger.log(
        `[공유 보드 변환] Vision 응답: ${visionResult.text.substring(0, 100)}...`,
      );
      ...
      try {
        ...
        this.logger.warn(
          `[공유 보드 변환] JSON 파싱 실패: ${parseError.message}`,
        );
  ...
  private async handleQuizGenerate(
    ...
    this.logger.log(
      `[스터디 퀴즈] 생성 시작 - ${questionCount}문제, 유형: ${quizType}, QA히스토리: ${qaHistory.length}개`,
    );
...
- JSON만 출력하세요 (다른 텍스트 없이)`;
      ...
      this.logger.log(
        `[스터디 퀴즈] ${source} 기반 LLM 응답: ${response.substring(0, 100)}...`,
      );
    ...
    try {
        ...
        try {
          ...
          const ragQueryPromise = (async () => {
            ...
            if (
              ...
              this.logger.log(
                `[스터디 퀴즈] RAG 컨텍스트 ${ragResult.sources.length}개 조회됨`,
              );
      ...
      if (ragBasedQuestions && ragBasedQuestions.length > 0) {
        ...
        throw new Error(
          "퀴즈 생성할 컨텐츠가 없습니다. 문서를 검색하거나 회의를 진행해주세요.",
        );
      }
      ...
      this.logger.log(
        `[스터디 퀴즈] ${questions.length}개 문제 생성 완료 (출처: ${source})`,
      );
    } catch (error) {
  ...
  private async handleQuizEvaluate(
    ...
    this.logger.log(
      `[스터디 퀴즈] 평가 시작 - 질문: "${question?.question?.substring(0, 30)}...", 답변: "${userAnswer.substring(0, 30)}..."`,
    );
...
- 시간 초과나 답변 없음은 오답 처리`;
      ...
      this.logger.log(
        `[스터디 퀴즈] 평가 응답: ${response.substring(0, 100)}...`,
      );
      ...
      try {
        ...
        this.logger.warn(
          `[스터디 퀴즈] 평가 JSON 파싱 실패: ${parseError.message}`,
        );
        // 단순 비교로 폴백
        ...
        evaluation.studyGuide = !evaluation.isCorrect
          ? question?.explanation || ""
          : "";
      }
      ...
      this.logger.log(
        `[스터디 퀴즈] 평가 완료 - 정답: ${evaluation.isCorrect}`,
      );
    } catch (error) {
      ...
      const evalResult = {
        ...
        evaluation: {
          ...
          feedback: isCorrect
            ? "정답입니다!"
            : `틀렸습니다. 정답은 "${question?.correctAnswer}"입니다.`,
          studyGuide: !isCorrect
            ? question?.explanation || "해당 내용을 다시 복습해보세요."
            : "",
        },
      };
  ...
  private async handlePerplexityMessage(
    ...
    const isSpeakingOrPublishing =
      context.botState === BotState.SPEAKING || context.isPublishing;
    ...
    try {
      ...
      const result = await this.perplexityService.handleMessage(
        roomId,
        message,
      );
      ...
      if (result?.action) {
        ...
        if (result.response) {
          this.logger.log(
            `[Perplexity] AI 응답: "${result.response.substring(0, 50)}..."`,
          );
  ...
  private async handleCDRFlowchartRequest(
    ...
    this.logger.log(
      `[CDR→Flowchart] 요청 처리 from ${requesterId}: ${content.length}자`,
    );
...
{"entities":[...],"actions":[...],"conditions":[...],"relations":[...]}`;
      ...
      this.logger.log(
        `[Domain Model] 추출 시작 (${processedContent.length}자)`,
      );
      const answer = await this.llmService.sendMessagePure(prompt, 2000);
      ...
      this.logger.log(
        `[Domain Model 완료] 엔티티: ${domainModel.entities.length}, 액션: ${domainModel.actions.length}`,
      );
  ...
  private generateERDiagram(domainModel: {
    ...
    try {
      ...
      for (const rel of domainModel.relations) {
        const fromEntity = domainModel.entities.find(
          (e) => e.name === rel.from,
        );
      ...
      this.logger.log(
        `[ER Diagram] 생성 완료 (${domainModel.entities.length}개 엔티티)`,
      );
  ...
  private async handleCDRAnalysisRequest(
    ...
    this.logger.log(
      `[CDR 분석] 종합 분석 시작 from ${requesterId}: ${content.length}자`,
    );
    ...
    const responseMessage = {
      ...
      domainModel: domainModelResult.success
        ? domainModelResult.domainModel
        : null,
    ...
    this.logger.log(
      `[CDR 분석] 응답 전송 - DM: ${domainModelResult.success}, FC: ${flowchartResult.success}, ER: ${erDiagramResult.success}`,
    );
  }
  ...
  private async generateSkeletonCode(
    ...
    try {
      const flowchartDesc = nodes
        .map((n) => `- ${n.id}: ${n.content} (${n.nodeType})`)
        .join("\n");
...
코드만 출력 (설명 없이):`;
  ...
  private async handleSkeletonCodeRequest(
    ...
    this.logger.log(
      `[Skeleton] 요청 처리 from ${requesterId}: ${nodes.length}개 노드, ${language}`,
    );
    ...
    this.logger.log(
      `[Skeleton] 응답 전송: ${result.success ? "성공" : "실패"}`,
    );
  }
  ...
  private async handleAudioTrack(
    ...
    for await (const frame of audioStream) {
      ...
      if (isCalibrating) {
        ...
        if (elapsed < CALIBRATION_DURATION) {
          ...
          if (calibrationSamples.length > 0) {
            backgroundNoiseDB =
              calibrationSamples.reduce((sum, db) => sum + db, 0) /
              calibrationSamples.length;
            MIN_DECIBEL_THRESHOLD = backgroundNoiseDB + VOICE_MARGIN_DB;
            const noiseRMS = 32768 * Math.pow(10, backgroundNoiseDB / 20);
            VOICE_AMPLITUDE_THRESHOLD = Math.max(
              300,
              Math.min(800, noiseRMS * 3),
            );
            this.logger.log(
              `[캘리브레이션] 배경 소음: ${backgroundNoiseDB.toFixed(1)}dB`,
            );
          }
          isCalibrating = false;
        }
      ...
      if (!isCalibrating && decibel < MIN_DECIBEL_THRESHOLD - 5) {
        backgroundNoiseDB =
          backgroundNoiseDB * (1 - NOISE_UPDATE_ALPHA) +
          decibel * NOISE_UPDATE_ALPHA;
      }

      const isVoice =
        avgAmplitude > VOICE_AMPLITUDE_THRESHOLD &&
        decibel > MIN_DECIBEL_THRESHOLD;

      // 화자 음성 레벨 학습
      if (!isCalibrating && isVoice) {
        speakerVoiceDB =
          speakerVoiceDB * (1 - SPEAKER_UPDATE_ALPHA) +
          decibel * SPEAKER_UPDATE_ALPHA;
        speakerSampleCount++;

        if (speakerSampleCount >= MIN_SPEAKER_SAMPLES) {
          const optimalThreshold =
            backgroundNoiseDB + (speakerVoiceDB - backgroundNoiseDB) * 0.3;
          if (Math.abs(optimalThreshold - MIN_DECIBEL_THRESHOLD) > 2) {
            MIN_DECIBEL_THRESHOLD = optimalThreshold;
            const thresholdRMS =
              32768 * Math.pow(10, MIN_DECIBEL_THRESHOLD / 20);
            VOICE_AMPLITUDE_THRESHOLD = Math.max(
              300,
              Math.min(800, thresholdRMS * 1.5),
            );
          }
        }
      }
      ...
      if (context?.isPublishing && context.publishingStartTime > 0) {
        ...
        if (publishingDuration > PUBLISHING_TIMEOUT_MS) {
          this.logger.warn(
            `[오디오] isPublishing 타임아웃 (${(publishingDuration / 1000).toFixed(1)}초) - 강제 리셋`,
          );
          context.isPublishing = false;
          context.publishingStartTime = 0;
        }
      ...
      if (context?.isPublishing && context.botState === BotState.SPEAKING) {
        ...
        if (
          isVoice &&
          voiceCount >= MIN_VOICE_FRAMES &&
          !context.shouldInterrupt &&
      ...
      const triggeredBySilence =
        silenceCount > SILENCE_THRESHOLD && totalLength > MIN_AUDIO_LENGTH;
      ...
      if (shouldProcess && totalLength > MIN_AUDIO_LENGTH) {
        ...
        const triggerReason = triggeredByMaxLength
          ? "⚠️MAX_LENGTH"
          : "✅SILENCE";
        this.logger.log(
          `[VAD] ${triggerReason} | 오디오: ${audioSec}초 (${fullAudio.length} bytes)`,
        );

        const fullSamples = new Int16Array(
          fullAudio.buffer.slice(
  ...
  private async processAndRespond(
    ...
    try {
      const sttResult = await this.sttService.transcribeFromBufferStreamWithRaw(
        audioBuffer,
        "live-audio.pcm",
      );
    ...
    this.logger.log(
      `[1.STT] ${Date.now() - sttStart}ms - "${transcript}" (${userId})`,
    );
    ...
    if (context.perplexityModeActive && transcript.trim().length > 0) {
      this.logger.debug(
        `[Perplexity] Transcript 저장 시도: "${transcript.substring(0, 30)}..."`,
      );
      ...
      const transcriptResult = await this.perplexityService.handleMessage(
        roomId,
        transcriptMessage,
      );
      ...
      if (
        ...
        this.logger.log(
          `[Perplexity] 텍스트 기반 AI 개입: ${transcriptResult.action?.type}`,
        );
    ...
    if (
      ...
      Date.now() - context.lastResponseTime < RESPONSE_COOLDOWN_MS
    ) {
      this.logger.log(
        `[스킵] 응답 쿨다운 (${(RESPONSE_COOLDOWN_MS - (Date.now() - context.lastResponseTime)) / 1000}초 남음)`,
      );
      return;
    }
    ...
    try {
      ...
      if (context.ideaModeActive) {
        this.logger.log(
          `[아이디어 모드] 처리 시작 - "${transcript.substring(0, 30)}..."`,
        );
      ...
      if (
        context.flowchartModeActive &&
        context.flowchartContext &&
        intentForContext.isCallIntent
      ) {
        this.logger.log(
          `[Flowchart 모드] 질문 처리 시작 - "${transcript.substring(0, 30)}..."`,
        );

        // Flowchart 컨텍스트를 포함한 프롬프트 생성
        const flowchartContextStr = this.buildFlowchartContextPrompt(
          context.flowchartContext,
        );
      ...
      if (context.designModeActive && intentForContext.isCallIntent) {
        ...
        this.logger.log(
          `[설계 모드] 질문 분석 - 다이어그램 관련: ${isDiagramQuestion}`,
        );

        if (isDiagramQuestion) {
          // 다이어그램 관련 질문 - 설계 보드 컨텍스트와 함께 응답
          this.logger.log(
            `[설계 모드] 다이어그램 질문 처리 - "${transcript.substring(0, 30)}..."`,
          );
          ...
          this.logger.log(
            `[설계 모드] 비-다이어그램 질문 스킵 - "${transcript.substring(0, 30)}..."`,
          );
          context.botState = BotState.SPEAKING;
          await this.speakAndPublish(
            context,
      ...
      this.logger.log(
        `[2.Intent] ${Date.now() - intentStart}ms - call=${intentAnalysis.isCallIntent}, conf=${intentAnalysis.confidence.toFixed(2)}, cat=${intentAnalysis.category}, needsLlm=${intentAnalysis.needsLlmCorrection}`,
      );
      ...
      // ================================================
      this.logger.debug(
        `[Vision 체크] isCallIntent=${intentAnalysis.isCallIntent}, isVisionIntent=${intentAnalysis.isVisionIntent}, hasScreenShare=${context.hasActiveScreenShare}, isVisionMode=${context.isVisionMode}`,
      );

      if (
        intentAnalysis.isCallIntent &&
        intentAnalysis.isVisionIntent &&
        context.hasActiveScreenShare
      ) {
      ...
      if (intentAnalysis.isCallIntent && intentAnalysis.isFlowchartIntent) {
        this.logger.log(
          `[Flowchart 보드 열기] Intent 감지 - 발화자: ${userId}`,
        );
        ...
        this.logger.log(
          `[Flowchart 보드] initiator 설정: ${userId}, openTime: ${openTime}`,
        );
      ...
      if (intentAnalysis.isCallIntent && intentAnalysis.isCalendarIntent) {
        ...
        try {
          ...
          this.logger.log(
            `[캘린더] 참여자 ${participantCount}명의 일정 분석 시작`,
          );
      ...
      if (intentAnalysis.isCallIntent && intentAnalysis.isTimerIntent) {
        this.logger.log(
          `[타이머] Intent 감지 - action: ${intentAnalysis.timerAction}, minutes: ${intentAnalysis.timerMinutes}`,
        );
        ...
        try {
          const {
            ...
            stopTimer,
            getTimerStatus,
          } = await import("../mcp/tools/timer.tool");
      ...
      if (!shouldRespond && intentAnalysis.needsLlmCorrection) {
        this.logger.log(
          `[3.LLM교정] 시작 (conf=${intentAnalysis.confidence.toFixed(2)})`,
        );
        const correctionStart = Date.now();
        const voiceIntent = await this.llmService.processVoiceIntent(
          transcript,
          intentAnalysis,
        );
        this.logger.log(
          `[3.LLM교정] ${Date.now() - correctionStart}ms - respond=${voiceIntent.shouldRespond}`,
        );
      ...
      if (context.botState === BotState.SLEEP) {
        ...
        // 질문이 있는지 확인
        const hasQuestion =
          intentAnalysis.isQuestionIntent ||
      ...
      if (context.botState === BotState.ARMED) {
        ...
        this.broadcastAiState(roomId, "listening", {
        ...
        // ============================================
        const useAgentMode = this.shouldUseAgentMode(
          processedText,
          intentAnalysis,
        );
        ...
        if (!finalSearchKeyword || finalSearchKeyword.length < 2) {
          ...
          const searchPlan =
            await this.llmService.buildSearchPlan(processedText);
          finalSearchKeyword = searchPlan.query;
          finalCategory = searchPlan.category || category;
        }

        this.logger.log(
          `[검색 준비] keyword="${finalSearchKeyword}", cat=${finalCategory}`,
        );

        // ★ AI 상태: processing (시리 UI용)
        this.broadcastAiState(roomId, "processing", {
          transcript: processedText,
        }).catch(() => {});
        ...
        const getThinkingPhrase = (category: string | null): string => {
          ...
          if (
            ...
            return ["어디 좋을까... 잠깐만요", "음 찾아볼게요~", "어 잠깐요~"][
        ...
        const llmPromise = this.llmService
          .sendMessage(processedText, intentAnalysis.searchDomain, roomId)
          .finally(() => {
            llmResolved = true;
          });
        ...
        this.logger.log(
          `[4.LLM] ${Date.now() - llmStart}ms - "${llmResult.text.substring(0, 50)}..."`,
        );
        ...
        if (llmResult.searchResults && llmResult.searchResults.length > 0) {
          ...
          if (context.hostOnlyMode && context.hostIdentities.size > 0) {
            ...
            this.logger.log(
              `[DataChannel] 검색 결과 호스트들에게 전송 (${llmResult.searchResults.length}개)`,
            );
          } else {
            // 모든 참여자에게 전송
            await context.room.localParticipant.publishData(
            ...
            this.logger.log(
              `[DataChannel] 검색 결과 전송 (${llmResult.searchResults.length}개)`,
            );
          }
        }
        ...
        if (llmResult.ragSources && llmResult.ragSources.length > 0) {
          ...
          this.logger.log(
            `[DataChannel] AI 문서 패널 전송 (${llmResult.ragSources.length}개 출처)`,
          );
        }
        ...
        const hasSearchResults =
          llmResult.searchResults && llmResult.searchResults.length > 0;
        ...
        this.logger.log(
          `========== [완료] 총 ${Date.now() - responseStartTime}ms ==========\n`,
        );
      }
    } catch (error) {
      ...
      const sendFrame = () => {
        ...
        if (frameIndex < frames.length) {
          ...
          const nextDelay = Math.max(
            1,
            FRAME_DURATION_MS - (actualTime - expectedTime + FRAME_DURATION_MS),
          );
  ...
  private async speakAndPublish(
    ...
    if (context.aiMuted) {
      this.logger.log(
        `[TTS 스킵] AI 음소거 상태 - 메시지: "${message.substring(0, 50)}..."`,
      );
      return;
    }
    ...
    if (context.hostOnlyMode && context.hostIdentities.size > 0) {
      ...
      this.logger.log(
        `[텍스트 카드] 호스트에게만 전송 - 메시지: "${message.substring(0, 50)}..."`,
      );
    ...
    this.logger.log(
      `[TTS] ${Date.now() - ttsStart}ms - ${pcmAudio.length} bytes`,
    );
    ...
    if (
      context.currentRequestId !== requestId &&
      context.currentRequestId !== 0
    ) {
      this.logger.warn(
        `[TTS 스킵] requestId 불일치: current=${context.currentRequestId}, expected=${requestId}`,
      );
      return;
    }
  ...
  private shouldUseAgentMode(
    ...
    if (
      ...
      ["날씨", "맛집", "카페", "술집", "뉴스", "주식", "스포츠"].includes(
        intentAnalysis.category,
      )
    ) {
      return false;
    ...
    if (
      intentAnalysis.extractedKeyword &&
      intentAnalysis.extractedKeyword.length >= 3
    ) {
      return false;
    }
  ...
  private async processWithAgent(
    ...
    try {
      ...
      if (screenContext) {
        this.logger.log(
          `[Agent] 화면 컨텍스트 포함 (${context.screenContextHistory?.length || 0}회 캡처)`,
        );
      }
      ...
      this.logger.log(
        `[Agent] Tool: ${decision.tool}, Params: ${JSON.stringify(decision.params)}`,
      );
      ...
      if (toolResult.searchResults && toolResult.searchResults.length > 0) {
        ...
        this.logger.log(
          `[Agent] DataChannel 전송 (${toolResult.searchResults.length}개 결과)`,
        );
      }
  ...
  private updateConversationHistory(
    ...
    if (cleanedUserMessage && cleanedUserMessage.length >= 3) {
      ...
      this.logger.debug(
        `[대화 히스토리] 사용자 발화 스킵 (짧거나 추임새): "${userMessage}"`,
      );
    }
    ...
    if (context.conversationHistory.length > this.MAX_CONVERSATION_TURNS * 2) {
      context.conversationHistory = context.conversationHistory.slice(
        -this.MAX_CONVERSATION_TURNS * 2,
      );
    }

    this.logger.debug(
      `[대화 히스토리] ${context.conversationHistory.length}턴 저장`,
    );
  }
  ...
  private saveDebugAudio(audioBuffer: Buffer, roomId: string): void {
    try {
      ...
      this.logger.log(
        `[DEBUG] 오디오 저장: ${filename} (${audioBuffer.length} bytes)`,
      );
    } catch (err) {
  ...
  private async cleanupRoomInDatabase(roomId: string): Promise<void> {
    try {
      ...
      this.logger.log(
        `[DB 정리 완료] roomId: ${roomId}, result: ${JSON.stringify(response.data)}`,
      );
    } catch (error) {
      this.logger.error(
        `[DB 정리 실패] roomId: ${roomId}, error: ${error.message}`,
      );
    }
  }
  ...
  async handleVisionCaptureFromHttp(
    ...
    this.logger.log(
      `[Vision HTTP] 처리 시작 - room: ${roomId}, requestId: ${message.requestId}`,
    );
    await this.handleVisionCaptureResponse(roomId, message);
  }
  ...
  private async handleTimerCommand(
    ...
    switch (action) {
      case "start": {
        ...
        const warnMins =
          warningMinutes || Math.max(1, Math.floor(targetMins * 0.15));
        result = startTimer(roomId, targetMins, warnMins);
        this.logger.log(
          `[타이머] 시작: ${targetMins}분, 경고: ${warnMins}분 전`,
        );
        break;
      }
      case "pause":
  ...
  private async handleVisionCaptureResponse(
    ...
    if (message.requestId !== requestId) {
      this.logger.warn(
        `[Vision] 요청 ID 불일치: ${message.requestId} vs ${requestId}`,
      );
      return;
    }
    ...
    this.logger.log(`\n========== [Vision 캡처 응답 처리] ==========`);
    this.logger.log(
      `이미지 크기: ${(message.imageBase64.length / 1024).toFixed(1)}KB`,
    );
    if (message.cursorPosition) {
      this.logger.log(
        `마우스 커서: (${message.cursorPosition.x}, ${message.cursorPosition.y})`,
      );
    } else {
      this.logger.log(`마우스 커서: 없음`);
    }
    this.logger.log(
      `화면 크기: ${message.screenWidth}x${message.screenHeight}`,
    );

    try {
      // 이미지 검증
      const validation = this.visionService.validateAndCompressImage(
        message.imageBase64,
      );
      if (!validation.valid) {
        ...
        await this.speakAndPublish(
          ...
          roomId,
          requestId,
          "화면을 캡처하는데 문제가 있어요. 다시 한번 말씀해주세요.",
        );
        return;
      }
      ...
      if (!context.hostOnlyMode) {
        const thinkingPhrases = ["잠깐 볼게요~", "한번 볼게요", "잠깐만요~"];
  ...
  async processVisionIntent(
    ...
    if (context.isVisionMode && context.pendingVisionRequest) {
      ...
      if (elapsed < 10000) {
        this.logger.log(
          `[Vision] 이미 처리 중 (${(elapsed / 1000).toFixed(1)}초 경과) - 스킵`,
        );
        return false;
      }
      // 10초 이상 응답 없으면 리셋하고 새로 시작
      this.logger.log(
        `[Vision] 이전 요청 타임아웃 (${(elapsed / 1000).toFixed(1)}초) - 리셋 후 재시도`,
      );
      context.isVisionMode = false;
      context.pendingVisionRequest = undefined;
    }
    ...
    // 캡처 요청 전송
    await this.sendVisionCaptureRequest(
      roomId,
      requestId,
      userQuestion,
      userId,
  ...
  private async explainGeneratedDiagrams(
    ...
    this.logger.log(
      `[설계 모드] 다이어그램 자동 설명 시작 - ${types.join(", ")}`,
    );
...
설명:`;
  ...
  private async flushTimelineKeywords(roomId: string): Promise<void> {
    ...
    try {
      // 발화자별로 키워드 추출 (병렬 처리)
      const speakerKeywordsPromises = Array.from(
        speakerTranscripts.entries(),
      ).map(async ([speaker, texts]) => {
      ...
      // 키워드가 하나도 없으면 스킵
      const validResults = speakerKeywordsResults.filter(
      ...
      const logSummary = validResults
        .map((r) => `${r.speaker}: [${r.keywords.join(", ")}]`)
        .join(" | ");
      this.logger.log(
        `[Timeline] 키워드 갱신 (minute: ${minuteIndex}) - ${logSummary}`,
      );
...
## 교정된 키워드 (최대 5개)`;
  ...
  private sendToRagForEmbedding(
    ...
    const meaningfulWords = words.filter(
      (word) => !fillerWords.has(word.toLowerCase()),
    );
    ...
    this.ragClient
      .sendStatement(roomId, text, speaker, startTime, endTime)
      .then(() => {
        this.logger.debug(
          `[RAG 임베딩] 전송 완료: "${text.substring(0, 30)}..." by ${speaker}`,
        );
      })
      .catch((err) => {
  ...
  private async checkAndOfferOpinion(
    ...
    try {
      ...
      if (!result.shouldSpeak || !result.response) {
        this.logger.debug(
          `[AI 의견] 제시 조건 미충족: ${result.silenceReason || "unknown"}`,
        );
        return false;
      }

      this.logger.log(
        `[AI 의견 제시] 신뢰도: ${result.confidence}%, 토픽: ${result.evidence?.topic}`,
      );
      ...
      if (result.evidence) {
        const evidenceMessage = this.opinionService.formatForDataChannel(
          result.evidence,
        );
        const encoder = new TextEncoder();
        await context.room.localParticipant.publishData(
        ...
        this.logger.log(
          `[DataChannel] Evidence 정보 전송: ${result.evidence.sourceName}`,
        );
      }
   ...
   */
  private async sendTextCardToHost(
    context: RoomContext,
    message: string,
  ): Promise<void> {
    ...
    this.logger.log(
      `[텍스트 카드] 호스트들에게 전송 완료 (${context.hostIdentities.size}명)`,
    );
  }
   ...
   */
  private async processHostQuery(
    roomId: string,
    query: string,
    hostIdentity: string,
  ): Promise<void> {
  ...
  buildScreenContextPrompt(roomId: string): string {
    ...
    prompt +=
      "사용자가 공유한 화면에서 추출한 텍스트입니다. 질문에 답할 때 이 정보를 참고하세요.\n\n";
  ...
  private async runProactiveAnalysis(
    ...
    // 비활성화 또는 컨텍스트 없으면 스킵
    if (
      !context.proactiveAnalysis.enabled ||
      !context.screenUnderstandingMode
    ) {
      return;
    }
    ...
    const currentScreenHash =
      this.proactiveAnalysisService.hashTexts(recentScreenTexts);
    ...
    const recentConversation = context.conversationHistory
      ...
      .map(
        (turn) =>
          `${turn.role === "user" ? "사용자" : "아우라"}: ${turn.content}`,
      );

    // 대화 해시 생성
    const currentConversationHash =
      this.proactiveAnalysisService.hashTexts(recentConversation);

    // 화면 AND 대화 모두 변경 없으면 스킵
    const screenChanged =
      currentScreenHash !== context.proactiveAnalysis.lastScreenTextHash;
    const conversationChanged =
      currentConversationHash !==
      context.proactiveAnalysis.lastConversationHash;
    ...
    if (conversationChanged) {
      this.logger.debug(
        `[Proactive] 대화 변경 감지 (${recentConversation.length}턴)`,
      );
    }
  ...
  private startSilentParticipantChecker(roomId: string): void {
    ...
    const checkInterval = setInterval(async () => {
      ...
      for (const participant of context.room.remoteParticipants.values()) {
        ...
        if (
          silentDuration >= SILENT_THRESHOLD_MS &&
          !alreadyAlerted.has(identity)
        ) {
          const participantName = participant.name || identity;
          await this.sendSilentParticipantAlert(
            roomId,
            identity,
            participantName,
            silentDuration,
          ...
          this.logger.log(
            `[Silent 감지] ${participantName} - ${Math.floor(silentDuration / 60000)}분 동안 발언 없음`,
          );
        }
  ...
  async sendSilentParticipantAlert(
    ...
    const context = this.activeRooms.get(roomId);
    if (!context || !context.hostOnlyMode || context.hostIdentities.size === 0)
      return;
    ...
    this.logger.log(
      `[Silent 알림] ${participantName} (${Math.floor(silentDurationMs / 60000)}분) → 호스트들에게 전송`,
    );
  }
  ...
  private startSpeakingStatsChecker(roomId: string): void {
    ...
    const statsInterval = setInterval(async () => {
      ...
      for (const stats of context.participantSpeakingStats.values()) {
        const ratio =
          totalDurationMs > 0 ? stats.speakingDurationMs / totalDurationMs : 0;
        statsArray.push({
      ...
      try {
        ...
        if (statsArray.length >= 2 && !context.dominantAlertSent) {
          ...
          if (topSpeaker.speakingRatio >= DOMINANT_THRESHOLD) {
            ...
            this.logger.log(
              `[Dominant 알림] ${topSpeaker.participantName} (${Math.round(topSpeaker.speakingRatio * 100)}%) → 호스트들에게 전송`,
            );
          }
        }
        ...
        if (statsArray.length >= 2 && context.dominantAlertSent) {
          const topSpeaker = statsArray[0];
          if (topSpeaker.speakingRatio < DOMINANT_THRESHOLD - 0.1) {
            // 50% 미만이면 리셋
