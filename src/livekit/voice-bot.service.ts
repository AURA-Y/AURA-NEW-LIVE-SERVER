...
export class VoiceBotService {
  ...
  async startBot(
    ...
    this.logger.log(`\n========== [AI 봇 시작] ==========`);
    this.logger.log(
      `방: ${roomName} (Title: ${roomTitle || "N/A"}), URL: ${livekitUrl}`
    );
    ...
    room.on(
      ...
      async (payload: Uint8Array, participant?: RemoteParticipant) => {
        try {
          ...
          this.logger.debug(
            `[DataChannel] 메시지 수신 from ${
              participant?.identity || "unknown"
            }: type=${message.type}`
          );
          if (message.type === "vision_capture_response") {
            this.logger.log(
              `[Vision] 캡처 응답 수신 - requestId: ${
                message.requestId
              }, 이미지 크기: ${(message.imageBase64?.length / 1024).toFixed(
                1
              )}KB`
            );
    ...
    room.on(RoomEvent.DataReceived, (payload: Uint8Array, participant: any) => {
      try {
        ...
        this.logger.debug(
          `[DataChannel 수신] type=${message.type}, from=${
            participant?.identity || "unknown"
          }`
        );
    ...
    try {
      ...
      try {
        ...
        if (roomTitle) {
          this.logger.log(`[RAG 컨텍스트] Title: ${roomTitle}`);
        }
...
JSON 배열만 출력:`;
    ...
    try {
      ...
      jsonStr = jsonStr.replace(
        /"content"\s*:\s*"([^"]+)"\s*,\s*"type"/g,
        '"content":"$1"},{"type"'
      );

      this.logger.debug(
        `[DDD LLM] 처리된 JSON: ${jsonStr.substring(0, 100)}...`
      );
  ...
  private async processAndRespond(
    ...
    if (
      ...
      this.logger.log(
        `[스킵] 응답 쿨다운 (${
          (RESPONSE_COOLDOWN_MS - (Date.now() - context.lastResponseTime)) /
          1000
        }초 남음)`
      );
      return;
    ...
    try {
      ...
      const transcript = await this.sttService.transcribeFromBufferStream(
        audioBuffer,
        "live-audio.pcm"
      );
      ...
      // ★ 모드 상태 디버그 로그
      this.logger.debug(
        `[모드 상태] ideaMode=${context.ideaModeActive}, eventStormMode=${context.eventStormModeActive}`
      );
      ...
      if (context.ideaModeActive) {
        ...
        await this.detectAndBroadcastIdea(
          roomName,
          context,
          transcript,
          userId
      ...
      if (context.eventStormModeActive) {
        ...
        if (context.lastDddText === transcript) {
          this.logger.debug(
            `[DDD] 중복 텍스트 - 스킵: "${transcript.substring(0, 20)}..."`
          );
          return;
        }
        ...
        try {
          this.logger.log(
            `[DDD 모드] 처리 시작 - "${transcript.substring(0, 30)}..."`
          );
          await this.analyzeAndBroadcastDDD(
            roomName,
            context,
            transcript,
            userId
      ...
      this.logger.log(
        `[2.Intent] ${Date.now() - intentStart}ms - call=${
          intentAnalysis.isCallIntent
        }, conf=${intentAnalysis.confidence.toFixed(2)}, cat=${
          intentAnalysis.category
        }, needsLlm=${intentAnalysis.needsLlmCorrection}`
      );
      ...
      if (intentAnalysis.isCallIntent && intentAnalysis.isDddBoardIntent) {
        ...
        await this.speakAndPublish(
          ...
          roomName,
          requestId,
          "이벤트 스토밍 보드를 열었습니다. 도메인 이벤트를 말씀해주세요!"
        );
        context.botState = BotState.ARMED;
        context.lastResponseTime = Date.now();
        return;
      ...
      if (!shouldRespond && intentAnalysis.needsLlmCorrection) {
        ...
        this.logger.log(
          `[3.LLM교정] ${Date.now() - correctionStart}ms - respond=${
            voiceIntent.shouldRespond
          }`
        );
      ...
      if (context.botState === BotState.ARMED) {
        ...
        if (!finalSearchKeyword || finalSearchKeyword.length < 2) {
          ...
          const searchPlan = await this.llmService.buildSearchPlan(
            processedText
          );
        ...
        const llmPromise = this.llmService
          .sendMessage(processedText, intentAnalysis.searchDomain, roomName)
          .finally(() => {
            llmResolved = true;
          });
        ...
        this.logger.log(
          `[4.LLM] ${Date.now() - llmStart}ms - "${llmResult.text.substring(
            0,
            50
          )}..."`
        );
  ...
  async handleVisionCaptureFromHttp(
    ...
    this.logger.log(
      `[Vision HTTP] 처리 시작 - room: ${roomName}, requestId: ${message.requestId}`
    );
    await this.handleVisionCaptureResponse(roomName, message);
  }
  ...
  async processVisionIntent(
    ...
    if (context.isVisionMode && context.pendingVisionRequest) {
      ...
      this.logger.log(
        `[Vision] 이전 요청 타임아웃 (${(elapsed / 1000).toFixed(
          1
        )}초) - 리셋 후 재시도`
      );
      context.isVisionMode = false;
