...
export class SttService {
    ...
    private shouldUseLlmCorrection(text: string): boolean {
        ...
        // 지역명 + 이상한 단어 조합이면 교정
        const hasLocation = this.LOCATION_HINTS.some(loc => lowerText.includes(loc.toLowerCase()));
        const hasWeirdWord = /성숙|캬페|마집|떡복/.test(lowerText);
        if (hasLocation || hasWeirdWord) {
            return true;
        }

        // 카테고리 키워드가 있으면 교정 (정확도 향상)
        const hasCategory = this.CATEGORY_HINTS.some(cat => lowerText.includes(cat.toLowerCase()));
        if (hasCategory) {
            return true;
        }
...
## 지역명 목록
${this.LOCATION_HINTS.join(', ')}

## 카테고리 목록
카페, 맛집, 술집, 분식, 치킨, 피자, 빵집, 디저트, 팝업, 전시, 날씨
