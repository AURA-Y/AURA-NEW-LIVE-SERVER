...
export class SttService implements OnModuleDestroy {
    ...
    private readonly PHONETIC_CORRECTIONS: Record<string, string> = {
        ...
        // 웨이크워드 오인식 (AURA - 영어 발음 스타일)
        '오라': '아우라',           // "AURA" 영어 발음 → 오라
        '오라야': '아우라야',
        '어라': '아우라',           // "aura" 빠른 영어 발음 → 어라
        '어라야': '아우라야',
        '아러': '아우라',           // 첫 음절 흐려질 때
        '아러야': '아우라야',
        '아 라': '아우라',          // 띄어서 인식
    ...
    private shouldUseLlmCorrection(text: string): boolean {
        ...
        // 웨이크워드 패턴이 있으면 교정 (아우라 + 영어 발음 변형)
        const wakePatterns = [
            // 한국어 발음 오류
            /아울라/, /아우나/, /아우러/, /아오라/, /오우라/, /어우라/,
            // 영어 발음 스타일 (AURA)
            /^오라/, /^어라/, /^아러/, /아으라/, /아아라/,
            // 아우라는 제외 (정확하면 교정 불필요)
        ];
...
## 자주 오인식되는 패턴
- "아울라야/아우나야/오우라야" → "아우라야" (한국어 발음)
- "오라야/어라야/아러야" → "아우라야" (영어 AURA 발음)
- "성숙" → "성수" (지역명인 경우만)
- "캬페/카패" → "카페"
