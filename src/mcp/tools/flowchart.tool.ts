...
function buildFocusedPrompt(keywords: string[]): string {
  return `당신은 회의 내용을 분석하여 플로우차트 데이터를 생성하는 전문가입니다.

다음은 회의에서 프로세스/워크플로우와 관련된 발화들입니다.
감지된 키워드를 중심으로 플로우차트를 생성해주세요.
...
1. 키워드를 중심으로 노드 생성
2. 발화 순서를 고려하여 흐름 연결
3. 조건문("만약", "~하면")은 decision 노드로 처리
4. 반복("반복", "~할 때까지")은 루프로 표현
...
export const flowchartTool: McpTool = {
  ...
  async execute(input: ToolInput, llmCall: LlmCallFn): Promise<ToolOutput> {
    ...
    const prompt = useFocused
      ...
      : FLOWCHART_PROMPT + input.transcript;
